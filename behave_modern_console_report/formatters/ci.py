        ...
        for feature in self._collector.execution.features:
            for scenario in feature.scenarios:
                if scenario.is_terminal:
                    ...
                    self._console.print(f"[{status}] {feature.name} / {scenario.name} ({format_duration(scenario.duration)})")
                    if self.formatter_config.show_steps:
                        for step in scenario.steps:
                            step_status = step.status.name.upper()
                            self._console.print(f"  [{step_status}] {step.keyword} {step.name} ({format_duration(step.duration)})")
